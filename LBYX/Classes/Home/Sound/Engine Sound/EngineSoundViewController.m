...
static NSString * const kUserDefaultsEngineSound = @"kUserDefaultsEngineSound";
@interface EngineSoundViewController ()
@property (nonatomic,copy)NSArray * arr_ZL;
@end

@implementation EngineSoundViewController
...
- (void)initDataSource
{
    self.title = LocalizedStaing(@"外部音");
    NSArray *tmpArray = @[
    ...
    NSArray *localMaxSoundArray = (NSArray *)[kUserDefaults objectForKey:kUserDefaultsEngineSound];
...
- (void)didSelectCellWithTableView:(UITableView *)tableView atIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSData * data1 = [[NSString stringWithFormat:@"06000100%@0000",self.arr_ZL[indexPath.row]] hexToData];
...
//    }];
    ...
    [kUserDefaults setObject:[SoundModel mj_keyValuesArrayWithObjectArray:self.dataSource] forKey:kUserDefaultsEngineSound];
