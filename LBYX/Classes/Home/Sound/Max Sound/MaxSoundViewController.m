...
static NSString * const kUserDefaultsMaxSound = @"kUserDefaultsMaxSound";
@interface MaxSoundViewController ()
@property (nonatomic,copy)NSArray * arr_ZL;
@end

@implementation MaxSoundViewController
- (void)viewDidLoad{
    [super viewDidLoad];
...
- (void)initDataSource
{
    ...
    NSArray *localMaxSoundArray = (NSArray *)[kUserDefaults objectForKey:kUserDefaultsMaxSound];
...
- (void)didSelectCellWithTableView:(UITableView *)tableView atIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSData * data1 = [[NSString stringWithFormat:@"08000100%@0000",self.arr_ZL[indexPath.row]] hexToData];
//    [BlueApi send:data1 withResponse:YES withSendStatusBlock:^(NSData *data) {
//
//    }];
    [BApi writeChar:data1 response:YES];
    [kUserDefaults setObject:[SoundModel mj_keyValuesArrayWithObjectArray:self.dataSource] forKey:kUserDefaultsMaxSound];
